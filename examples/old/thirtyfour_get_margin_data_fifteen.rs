// run prg
// RUST_LOG=debug cargo run --example thirtyfour_get_margin_data_fifteen 2>&1| tee -a  output_57.txt
...
const WEB_XPATH: &[&[&str]] = &[
    ...
    &["5",ACTION_CLICK,"click and remove so  iframe","//*[@id=\"main_content_container\"]/div[3]"],
...
async fn run() -> color_eyre::Result<(), Box<dyn Error>> {
    ...
    debug!("call save table_to_file");
    save_table_to_file_first(_driver.clone()).await?;
    debug !("Call close browser - inactive");
...
async fn wait_seconds_of_browser(
    ...
    debug!("Thread sleep for - START {} seconds",waiting_period);
    thread::sleep(Duration::from_secs(waiting_period));
    debug!("Thread sleep for - FINISHED {} seconds",waiting_period);
    Ok(())
}
